...
namespace AZ
{
    namespace Render
    {
        ...
        void DiffuseProbeGridComponent::Reflect(AZ::ReflectContext* context)
        {
            ...
            {
                serializeContext->Class<DiffuseProbeGridComponent, BaseClass>()->Version(0);
            }

            if (auto behaviorContext = azrtti_cast<BehaviorContext*>(context))
            {
                behaviorContext
                    ->ConstantProperty("DiffuseProbeGridComponentTypeId", BehaviorConstant(Uuid(DiffuseProbeGridComponentTypeId)))
